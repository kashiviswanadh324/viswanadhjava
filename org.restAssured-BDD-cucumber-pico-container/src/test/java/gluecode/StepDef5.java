...
public class StepDef5 {
	...
	public void method2()throws Exception {
		...
		eu.openSheet("sheet1");
		 ...
		 //Date driven from 2nd row(index is 1),because 1st row has name of column)
		 for(int i=1;i<nour;i++)
		 {
			 //put data to be update in json format request by parameter
			 Mypojo obj = new Mypojo();
			 ...
			 sh.res=sh.req.put(eu.getCellValue(i, 0));
			 // validate response 
			 if(sh.res.getStatusCode()==200 && eu.getCellValue(i, 4).equals("allvalid")) {
				 result.add("passed");
