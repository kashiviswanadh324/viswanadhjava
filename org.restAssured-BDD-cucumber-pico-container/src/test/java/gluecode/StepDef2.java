...
public class StepDef2 {
	...
	@Then("status code is {string} as per {string} in json response body") 
	 public void method1(String esc, String criteria) {
		...
		if(criteria.equals("valid") && sh.res.getStatusCode()==x)
		{
			sh.s.log("Test passed for criteria"+criteria);
			Assert.assertTrue(true);
		}else if(criteria.equals("invalid") && sh.res.getStatusCode()==x)
		{
			sh.s.log("Test passed for criteria"+criteria);
			Assert.assertTrue(true);
		}else if(criteria.equals("blank") && sh.res.getStatusCode()==x)
		{
			sh.s.log("Test passed for criteria"+criteria);
		}else {
			sh.s.log("Test failed for criteria"+criteria+"because expected is"+x+"but actual is"+sh.res.getStatusCode());
	...
	public void method3(String x, String y, String z) {
		...
		}else if(criteria.equals("wrong")&& sh.res.getStatusCode()==esc && sh.res.getContentType().contains(ect))
		{
			...
			sh.s.log("Test failed for criteria"+criteria+"because expected is"+esc+"but actual is"+sh.res.getStatusCode()+"and"+sh.res.getContentType());
			Assert.assertTrue(false);
		}
	}
